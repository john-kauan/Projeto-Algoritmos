...
void selection_menu(){
    ...
    do{
        printf("===SELECTION SORT - TIPO DE ENTRADA===\n");
        printf("c - Crescente\n");
            ...
            case 'r':
                do {
                    ...
                    if (tamanho_entrada >= 1 && tamanho_entrada <= 6) {
                        ...
                        char saida_path[256];
                        sprintf(saida_path, "Resultados/SelectionSort/Saidas/%s/Saida%s%d.txt", nome_tipo, nome_tipo, n);
                        salvar_dados(saida_path, vetor, n);

                        // Salvar o arquivo de tempo
                        char tempo_path[256];
                        sprintf(tempo_path, "Resultados/SelectionSort/Tempo/%s/Tempo%s%d.txt", nome_tipo, nome_tipo, n);
